        ...
        before(function() {
            ...
            return utils.common.waitLoader()
                ...
                .then(function(_draggedElementText_) {
                    ...
                    return browser
                        .actions()
                        .dragAndDrop(dragElement, dragableElements.get(0))
                        .perform()
                        .then(function() {
                            return utils.common.dragEnd();
                        })
                });
        });

        it('projects list has the new order', function() {
            var firstElement = element.all(by.css('.project-list-single a')).first().getText();

            expect(firstElement).to.be.eventually.equal(draggedElementText);
        });

        it.skip('projects menu has the new order', function() {
            browser.executeScript(function() {
                console.log( $('div[tg-dropdown-project-list] ul a').first().text());
            });

            // var firstElement = element(by.css('div[tg-dropdown-project-list] ul a')).getText();
